  ...
  const API = {
    ...
    get_video_metadata: async (video_id) => {
      ...
      for (const country of countries) {
        ...
        if ((media instanceof Object) && media.MediaURLs && Array.isArray(media.MediaURLs) && media.MediaURLs.length) {
          ...
          if (HLS_urls.length) {
            ...
            if (media.ShowName && (media.ShowName !== media.Title))
              series_title = media.ShowName

            if (media.Title)
              video_title = (media.Season && media.Episode)
                ? `S${pad_zeros(media.Season, 2)}E${pad_zeros(media.Episode, 2)} - ${media.Title}`
                : media.Title
